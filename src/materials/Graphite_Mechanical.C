...
// This kernel computes the thermophysical material properties of pure Mo
...
Graphite_Mechanical::Graphite_Mechanical(const InputParameters & parameters)
...
void
Graphite_Mechanical::computeStress()
{
...
void
Graphite_Mechanical::modifyStrainIncrement()
{
...
void
Graphite_Mechanical::computeThermalStrain(Real & thermal_strain_increment)
{
  Real LTE(0.0), LTE0(0.0);
  LTE = Graphite_linear_thermal_strain(_temperature[_qp]);
  LTE0 = Graphite_linear_thermal_strain(_temperature_old[_qp]);
...
bool
Graphite_Mechanical::updateElasticityTensor(SymmElasticityTensor & tensor)
{
  bool changed(false);
  Real YM(0.0);
  Real PR(0.0);
  ...
  YM = Graphite_elastic_modulus();
  //  YM = 204E9*(1-(6*_volumetric_irradiation_strain[_qp]));
  PR = Graphite_poissons_ratio();
  SM = Graphite_shear_modulus(YM, PR); //should be called after YM and PR are calculated
...
Real Graphite_shear_modulus(Real youngs_modulus, Real poissons_ratio)
{
